...
export default function RainOnGlass() {
    ...
    useEffect(() => {
        ...
        function generateCityBg(w: number, h: number, blurPx: number) {
            ...
            sky.addColorStop(0, '#0a0f1a'); sky.addColorStop(0.15, '#0f1a2a'); sky.addColorStop(0.35, '#1a2a3a');
            sky.addColorStop(0.55, '#2a3a4a'); sky.addColorStop(0.75, '#3a4a5a'); sky.addColorStop(1, '#4a5a6a');
            ctx.fillStyle = sky; ctx.fillRect(0, 0, w, h);

            const glow = ctx.createLinearGradient(0, h * 0.3, 0, h);
            glow.addColorStop(0, 'rgba(0,150,200,0)'); glow.addColorStop(0.3, 'rgba(0,180,220,0.2)');
            glow.addColorStop(0.6, 'rgba(0,200,255,0.35)'); glow.addColorStop(1, 'rgba(0,229,255,0.5)');
            ctx.fillStyle = glow; ctx.fillRect(0, 0, w, h);

            const bColors = ['#0a0f1a', '#0f1520', '#152030', '#1a2540'];
            for (let b = 0; b < 35; b++) {
                ...
                for (let wr = 0; wr < wRows; wr++) for (let wc = 0; wc < wCols; wc++) {
                    if (srand() > 0.45) {
                        ...
                        const warmth = srand();
                        ctx.fillStyle = warmth > 0.3 ? `rgba(0,229,255,${0.5 + srand() * 0.5})` : warmth > 0.1 ? `rgba(0,180,255,${0.4 + srand() * 0.4})` : `rgba(100,200,255,${0.3 + srand() * 0.3})`;
            ...
            for (let i = 0; i < 80; i++) {
                ...
                if (rndC < 0.45) { hue = 180 + srand() * 30; sat = 90 + srand() * 10; lit = 60 + srand() * 30; }
                else if (rndC < 0.7) { hue = 190 + srand() * 25; sat = 85 + srand() * 15; lit = 55 + srand() * 30; }
                else if (rndC < 0.85) { hue = 170 + srand() * 20; sat = 80 + srand() * 20; lit = 65 + srand() * 25; }
                else if (rndC < 0.93) { hue = 200 + srand() * 30; sat = 75 + srand() * 25; lit = 60 + srand() * 25; }
                else { hue = 185 + srand() * 20; sat = 85 + srand() * 15; lit = 65 + srand() * 20; }
        ...
        function render() {
            ...
            gl!.uniform1f(u.brightness, 1.35); gl!.uniform1f(u.alphaMultiply, 8.0); gl!.uniform1f(u.alphaSubtract, 2.0);
